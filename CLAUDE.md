...
- **Pod history tracking keys on UID + owner, not just pod name.** A crashed pod under a Deployment is deleted and replaced by a new Pod object (new name, new UID), so `(namespace, pod)` alone can't correlate a workload's history across that churn. `pod_metrics` also stores `pod_uid` and the pod's resolved owner (`owner_kind`/`owner_name`, following a ReplicaSet up to its own Deployment when present) — separate from `restart_count`, which only counts in-place container restarts within one pod's lifetime. Deliberately scoped to Kubernetes `ownerReferences` rather than Helm release labels: universal across every workload regardless of how it was deployed, at the cost of not grouping by Helm release (`app.kubernetes.io/instance`) out of the box, since that depends on chart convention, not a guaranteed API relationship.
